...
def gumbel_max_trick(x):
    ...
    u = -tf.log(-tf.log(tf.random_uniform(shape=[FLAGS.number_agents, FLAGS.vocabulary_size, FLAGS.batch_size],
                                          dtype=tf.float32)))
    utterance_output = tf.slice(x, [0, 2 * FLAGS.dim_env, 0],
                                [FLAGS.number_agents, FLAGS.vocabulary_size, FLAGS.batch_size])
...
def shuffle(x, name_targets, colors=False):
    ...
    batch_num = tf.tile(tf.reshape(tf.range(0, FLAGS.batch_size, dtype=tf.int32), [1, 1, FLAGS.batch_size]),
                        [FLAGS.number_agents,
                         1, 1])
    if not colors:
...
def compute_goal_dist(states, goal_location, goal_type):
    dist_positions = np.reshape(np.sqrt(np.sum((states[0:FLAGS.number_agents, 0:2, :] - goal_location) ** 2, axis=1)),
                                [FLAGS.number_agents, 1, FLAGS.batch_size])
    dist_gazes = np.reshape(np.sqrt(np.sum((states[0:FLAGS.number_agents, 4:6, :] - goal_location) ** 2, axis=1)),
                            [FLAGS.number_agents, 1, FLAGS.batch_size])
