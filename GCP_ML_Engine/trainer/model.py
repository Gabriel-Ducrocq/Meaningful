...
from trainer.src.util import print_stats_agent, compute_new_states, compute_new_memories, compute_reward, \
    delete_history_files, print_stat_vocabulary, memory
    ...
    def train(self, sess):
        ...
        for i in range(tf.app.flags.FLAGS.max_steps):
            ...
            if i % tf.app.flags.FLAGS.tensorboard_freq == 0:
                _, array_states, array_utterances, array_mem_com, array_mem_last, full_goals, array_outputs, t, reward, summary = sess.run(
                    self.output_to_run + [self.merged], feed_dict)
